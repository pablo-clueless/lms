...
import { useGetMySubmissions } from "@/lib/api/assignment";
...
const Page = () => {
  ...
  const columns = createAssignmentSubmissionColumns("STUDENT");

  const handleParamsChange = <K extends keyof PaginationParams>(field: K, value: PaginationParams[K]) => {
    setParams((prev) => ({ ...prev, [field]: value }));
  };

  const { data, isPending } = useGetMySubmissions(params);
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex w-full items-center justify-between">
        <div className="">
          <h3 className="text-foreground text-3xl">My Assignments</h3>
          <p className="text-sm text-neutral-600">View and submit your assignments</p>
        </div>
