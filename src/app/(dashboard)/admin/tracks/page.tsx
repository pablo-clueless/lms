...
import { DataTable, Loader, Pagination, Breadcrumb, CreateTrackDialog } from "@/components/shared";
import { createTrackColumns } from "@/config/columns";
import { useGetTracks, useCreateTrack } from "@/lib/api/cohort";
import { Button } from "@/components/ui/button";
import type { PaginationParams, Track } from "@/types";
import { cn } from "@/lib";

const breadcrumbs = [{ label: "Tracks", href: "/admin/tracks" }];
...
const Page = () => {
  ...
  const columns = createTrackColumns("ADMIN");
  const { data, isFetching, isPending, refetch } = useGetTracks(params);
  ...
  const handleCreateTrack = async (track: Partial<Track>) => {
    await createTrack.mutateAsync(track);
    setCreateOpen(false);
  };
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex w-full items-center justify-between">
        <div>
          <h3 className="text-foreground text-3xl">Tracks</h3>
          <p className="text-muted-foreground text-sm">Manage learning tracks and specializations</p>
        </div>
