...
import { useGetAssignments } from "@/lib/api/assignment";
...
const Page = () => {
  ...
  const columns = createAssignmentColumns("TUTOR");

  const handleParamsChange = <K extends keyof PaginationParams>(field: K, value: PaginationParams[K]) => {
    setParams((prev) => ({ ...prev, [field]: value }));
  };

  const { data, isFetching, isPending } = useGetAssignments(params);
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex w-full items-center justify-between">
        <div className="">
          ...
          <p className="text-sm text-neutral-600">Manage assignments for your courses</p>
        </div>
